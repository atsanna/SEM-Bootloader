...
void announceReply()
{
	...
	writePointer = spiReadWord(REG_S2_TX_WR0, 0) + S2_TX_START;
	txPtr = txBuffer + 2;

	// Send IP address in hex
	//
	for(i = REG_SIPR0; i <= REG_SIPR3; i++) {
		value = spiReadReg(i, 0);
		*txPtr++ = hex[value >> 4];
		*txPtr++ = hex[value & 0xf];
	}
	//
	...
	spiWriteWord(REG_S2_TX_WR0, 0, writePointer - S2_TX_START);
	spiWriteReg(REG_S2_CR, 0, CR_SEND);
	while(spiReadReg(REG_S2_CR, 0));
}
...
void announcePacket()
{
	...
	readPointer = spiReadWord(REG_S2_RX_RD0, 0) + S2_RX_START;

	// Read destination IP address
	for(count = 0; count < 4; count++) {
		spiWriteReg(REG_S2_DIPR0 + count, 0, readNextByte());
	}

	// Read destination port - but ignore it and respond on 5555 anyway.
	readNextByte();
	readNextByte();
	spiWriteWord(REG_S2_DPORT0, 0, ANNOUNCE_PORT);
	...
	spiWriteWord(REG_S2_RX_RD0, 0, readPointer - S2_RX_START); // Write back new pointer
	spiWriteWord(REG_S2_CR, 0, CR_RECV); // Receive again
...
void announceInit()
{
	// Open socket
	do {
		spiWriteWord(REG_S2_PORT0, 0, ANNOUNCE_PORT);
		spiWriteReg(REG_S2_MR, 0, MR_UDP);
		spiWriteReg(REG_S2_CR, 0, CR_OPEN);
		if(spiReadReg(REG_S2_SR, 0) != SOCK_UDP)
			spiWriteReg(REG_S2_CR, 0, CR_CLOSE);
	} while(spiReadReg(REG_S2_SR, 0) != SOCK_UDP);
...
void announcePoll()
{
	uint16_t packetSize = spiReadWord(REG_S2_RX_RSR0, 0);
